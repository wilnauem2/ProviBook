...
// Only checks explicit pool boolean fields — not string fields like zustellungsweg
const hasPool = (insurer, poolName) => {
  if (!insurer) return false;
  
  if (poolName.toUpperCase() === 'VEMA') {
    return insurer.vemapool === true;
  }
  if (poolName.toUpperCase() === 'FEMA') {
    return insurer.femapool === true;
  }
  return false;
};
...
const safeInsurers = computed(() => {
  if (!props.insurers || !Array.isArray(props.insurers)) return [];

  // Add status to each insurer
  let result = props.insurers.map(insurer => ({
    ...insurer,
  ...
  if (props.statusFilter) {
    ...
    result = result.filter(insurer => insurer.status === filterValue);
  }

  // Apply zustellungsweg filter if active
  if (activeZustellungswegFilter.value) {
    ...
    result = result.filter(insurer => {
      const zustellungsweg = getSafeZustellungsweg(insurer);
      const normalized = zustellungsweg ? zustellungsweg.toString().trim().toLowerCase() : 'nicht angegeben';
      return isNichtAngegeben
        ? !normalized || normalized === 'nicht angegeben' || normalized === ''
  ...
  if (activeDokumentenartFilter.value) {
    ...
    result = result.filter(insurer => {
      const dokumentenart = (insurer.dokumentenart || '').toString().trim().toLowerCase();
      return dokumentenart.includes(dokumentenartValue);
    });
  }
